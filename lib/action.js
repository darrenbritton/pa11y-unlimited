...
// Capability flag: this build surfaces real action failures ("was found but …",
// "is covered by another element") instead of masking them as "no element
// matching selector". Consumers can feature-detect it to know whether action
// errors are truthful.
 ...
 * no-element error; any other failure names the element as found and carries
 * the real reason through, so consumers can tell a missing element from an
 * action that failed on one that exists.
 * @param {*} error - The caught value.
...
async function findClickObstruction(handle) {
	try {
		...
		return await handle.evaluate(element => {
			...
			// Containment is checked across shadow boundaries too, so a
			// target that is itself a shadow host is not reported as
			// covered by its own shadow children.
			...
			if (!hit || hit === element || element.contains(hit) || hit.contains(element) ||
				isWithin(hit, element) || isWithin(element, hit)) {
				return null;
			}
...
module.exports.actions = [
	...
	// E.g. "click .sign-in-button"
	{
		...
		run: async (browser, page, options, matches) => {
			...
			// from "the element is there but the click failed", which used to
			// collapse into the same no-element error.
